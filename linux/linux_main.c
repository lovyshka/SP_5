            ...
            if (arr_len > 2){
                ...
                if (flag_child == 0){
                    ...
                    for (int j = 0; j < number_of_child; j++){       
                        char * args[4] = {"./subproc", generate_input_filename(j), generate_output_filename(j), NULL};
                        if (fork() == 0){
...
int * divided_properly(int number_of_child, int number_of_numbers, int * to_much_child){
    ...
    if (number_of_child > ((float) number_of_numbers) / 2.0){ //проверка на аборт
        *to_much_child = 1;
    }
    else if (number_of_numbers % number_of_child == 0){ //если можно по братски поделить
